 ...
 *“开始游戏”  左上（width /3, height/3）右下（width /3+280, height/3+100）
 *“游戏规则”  左上（width /3, height*19/30）右下（width /3+280,height*19/30+100）
 *“历史记录”  (width/3, height/5)
 ...
 * @param currentBaseHealth : 输入参数，当前基地血量（需传入实时血量，用于计算扣血后的值）
 ...
 */
 //基地扣血及累计伤害
void BaseTakeDamageAndAccumulate(int currentBaseHealth, int damageAmount, int* newBaseHealth, int* accumulatedDamage);
 ...
 * @param currentBaseHealth : 输入参数，当前基地血量（需传入实时血量，用于判断是否为0以触发结束条件）
 ...
 */
 //判断结束条件
void CheckGameRuleAndEndCondition(int currentBaseHealth, bool* isGameOver, char* gameResult);
...
*/
//计算累加游戏积分
void CalculateAndAddScore(int currentTotalScore, int scoreAmount, int* newTotalScore);
