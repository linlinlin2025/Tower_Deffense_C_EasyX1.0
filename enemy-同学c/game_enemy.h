...
struct enemy {
	...
	int current_speed;//速度
	int lifecount;//生命值
	int enemy_number;//发起进攻的敌人的数量

	int current_level;//敌人等级
...
struct Points Point_x_y(int random_num);//敌人初始位置选择
struct Points_2 current_move_x_y();//当前敌人移动的坐标
Dead_enemy_count Deadenemy_count(int ifshoot);//死亡敌人数给D
int Deadenemy_level(int point);//D给我当前积分数，我判断后变更等级，返回死亡敌人等级数给D
int Deadenemy_money(int deadenemy_num);//死亡敌人返回的总钱数给A
